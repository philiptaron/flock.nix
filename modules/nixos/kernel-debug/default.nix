# Kernel debugging patches for development and instrumentation.
# Import this module to add logging to crypto algorithm registration
# and usermode helper execution.
{ ... }:

{
  boot.kernelPatches = [
    {
      name = "crypto_larval_add logs when adding an algorithm";
      patch = ./crypto_larval_add-logging.patch;
    }
    {
      name = "user-mode helper subsystem logs when it runs something";
      patch = ./umh-logging.patch;
    }
  ];
}
